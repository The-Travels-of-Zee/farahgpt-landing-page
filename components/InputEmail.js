  ...
  return (
    <form onSubmit={subscribe} className="relative">
      <label className="block">
        <span className="block text-gray-800 text-lg font-semibold mb-2">Join the Waitlist</span>
        <div className="relative">
          ...
          <button
            type="submit"
            disabled={status === "loading"}
            className="absolute right-1 top-1 bottom-1 my-auto h-[calc(100%-8px)] px-4 bg-primary text-white font-semibold text-sm rounded-full hover:bg-primary/90 transition"
          >
            {status === "loading" ? "Subscribing..." : title}
          </button>
        </div>
        <p className="text-sm text-gray-500 mt-2">We’ll email you when we launch. No spam.</p>
      </label>
